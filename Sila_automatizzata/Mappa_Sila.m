...
z_grid_lim = 20:10:150;   % Quota di volo variabile AGL (metri sopra il terreno locale)
...
% Parametri Elettromagnetici arbitrari
frequenza = 2.4e9; 
Pt_drone = 30; %dBm (quanto è potente il segnale)
Gt = 2; %dBi (quanto efficacemente l'antenna focalizza un segnale in una direzione)
Gr = 2; 
...
% Il drone deve garantire il segnale a tutti i soccorritori, pertanto la
% logica utilizzata è quella Min-Max, in cui una volta individuato il 
% soccorritore messo peggio ci si pone l'obiettivo di massimizzare quella
% potenza minima. Dunque tutta la ricerca si basa sul trovare la posizione
% in cui il soccorritore svantaggiato ha comunque il miglior segnale possibile

disp('Applicazione Grid Search');

% Matrice 2D per raccogliere la miglior potenza per ogni coordinata X,Y 
% (Serve per mantenere compatibile la Heatmap)
mappaCopertura_2D = zeros(length(y_grid_lim), length(x_grid_lim));

% Inizializzazione variabili per il Massimo Globale Assoluto. La scelta di
% -Inf serve per salvare il primo valore utile come massima potenza
MaxPotenza_Ottimale = -Inf;
drone_X = 0; 
drone_Y = 0; 
drone_altezzaVolo = 0;

% Questi due cicli for servono a scannerizzare la griglia facendo spostare
% il drone. Prima di farlo alzare in volo, viene calcolata anche la quota
% del terreno in quello specifico punto, mentre miglior potenza verticale
% serve a trovare l'altezza migliore solo per quel singolo punto.
for idx_x = 1:length(x_grid_lim)
    for idx_y = 1:length(y_grid_lim)
        ...
        % Nel terzo for facciamo volare a diverse altitudini il drone, dove
        % test Z rappresenta l'altitudine assoluta rispetto al livello del mare
        for idx_z = 1:length(z_grid_lim)
            test_altezzaVolo = z_grid_lim(idx_z);
            test_Z = quota_terreno_locale + test_altezzaVolo;
            
            % Qui vengono calcolati i valori di potenza ricevuta dei
            % soccorritori, ma viene salvato il valore più basso per 
            % la logica del min_max fairness
            potenza_min_garantita = calcolaCopertura(test_X, test_Y, test_Z, ...
                                            socc_X, socc_Y, socc_Z, F_dem, frequenza, Pt_drone, Gt, Gr);
            
            % Salvo il valore massimo riscontrato su questa specifica verticale
            if potenza_min_garantita > miglior_potenza_verticale
                miglior_potenza_verticale = potenza_min_garantita;
            end
            
            % Controllo se questa terna (X,Y,Z) rappresenta il massimo globale assoluto
        ...
        % Qui il programma prende la miglior potenza verticale e la salva
        % nella matrice mappa copertura 2D nel pixel corrispondente, in
        % modo poi da poter generare in seguito una heatmap precisa
        mappaCopertura_2D(idx_y, idx_x) = miglior_potenza_verticale;
    end
end

% Calcolo delle coordinate Z assolute finali del drone ottimale
z_terreno_drone = F_dem(drone_X, drone_Y);
drone_Z = z_terreno_drone + drone_altezzaVolo;

fprintf('-> Posizione OTTIMALE 3D individuata autonomamente:\n');
...
% Ricalcolo e tratteggio dei raggi dalla posizione ottimale finale
for i = 1:length(socc_X)
    ...
    if all(clearance >= 0)
        ...
        fprintf('Soccorritore %d: LoS LIBERA. Rx: %.2f dBm\n', i, PotenzaRx);
    else
        coloreRaggio = 'r'; 
        ...
        fprintf('Soccorritore %d: OSTRUITA (Ostacolo %.2fm). Rx: %.2f dBm\n', i, h_ostacolo, PotenzaRx);
    end
    
    plot3(raggio_X, raggio_Y, raggio_Z, 'Color', coloreRaggio, 'LineWidth', 1);
end

title('Posizionamento Ottimale del Drone (Grid Search 3D)');
...
visualizzaHeatmapSAR(x_grid_lim, y_grid_lim, mappaCopertura_2D, socc_X, socc_Y, drone_X, drone_Y);