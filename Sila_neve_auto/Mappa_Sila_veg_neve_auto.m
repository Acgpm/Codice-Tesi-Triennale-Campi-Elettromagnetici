%  STEP 3: OTTIMIZZAZIONE POSIZIONE DRONE (GRID SEARCH 3D: OROGRAFIA + VEGETAZIONE + NEVE)
clc; clear; close all;
...
%% 3. ALGORITMO GRID SEARCH 3D
disp('Avvio Grid Search 3D (Orografia + Vegetazione + Neve)... Attendere.');
...
for idx_x = 1:length(x_grid_lim)
    for idx_y = 1:length(y_grid_lim)
        ...
        for idx_z = 1:length(z_grid_lim)
            ...
            potenza_min_garantita = calcolaCoperturaRETneve(test_X, test_Y, test_Z, ...
...
figure('Name', 'Setup ambiente ottimizzato', 'Color', 'w', 'Position', [100, 100, 900, 600]);
...
for i = 1:length(socc_X)
    ...
    text(sX, sY, sZ + 15, [' S', num2str(i)], 'FontWeight', 'bold', 'Color', 'r');
    ...
    if all(clearance >= 0)
        ...
        fprintf('S%d: LoS LIBERA. Rx Base: %.2f dBm ', i, PotenzaRx);
    else
        coloreRaggio = 'r'; 
        ...
        d2 = distanza_3D - d1;
        PotenzaRx = calcolaFriis(distanza_3D, frequenza, Pt_drone, Gt, Gr) + calcolaKnifeEdge(h_ostacolo, d1, d2, frequenza);
        fprintf('S%d: OSTRUITA. Rx Parz: %.2f dBm ', i, PotenzaRx);
    end
